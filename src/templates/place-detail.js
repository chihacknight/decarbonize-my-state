...
const PlaceDetailPage = ({location, data}) => {
  const filteredData = data.allFinalJson.edges.find(edge => !!edge.node.emissions).node.emissions
  const emissions_2018 = get_2018_emissions(filteredData)
  return (
    <Layout>
      <SEO />
      <PlaceDetail location={location} placesData={getPlacesData(filteredData)} emissions={emissions_2018}/>
    </Layout>
  )
}
